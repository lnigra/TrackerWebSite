...
", an end of file or run out of room then stop. */
	  /* this is our packet */
	  for (i=0;(!(feof(fpi))&&((packet[i]=fgetc(fpi))!=13)&&(i<MAXPACKETLEN));i++);
	  ...
	  if ((strstr(packet,"\nKC9LHW-11")!=NULL)||(strstr(packet,"\nKC9LIG-11")!=NULL)||(strstr(packet,"\nWB9SKY-11")!=NULL))
	    {
			fp1=fopen("kc9lhwtrack.txt","a");
			fp2=fopen("kc9ligtrack.txt","a");
			fp3=fopen("wb9skytrack.txt","a");
			printf("One of ours! \n");
			printf("Packet %s\n",packet);
	      ...
	      fprintf(fpo,"%d\n",Altitude);
			...
			fclose(fp2);      
			fclose(fp3);
			system("cp kc9lhwtrack.txt /Users/Dan/Desktop/7_6_2013FarHorizons/Test/Sites");
			system("cp kc9ligtrack.txt /Users/Dan/Desktop/7_6_2013FarHorizons/Test/Sites");
			system("cp wb9skytrack.txt /Users/Dan/Desktop/7_6_2013FarHorizons/Test/Sites");
			printf("DATA!");
			sleep(1);
